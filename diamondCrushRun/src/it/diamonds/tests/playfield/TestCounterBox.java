...
public class TestCounterBox extends TestCase
{
    ...
    public void testCorrectTexture()
    {
        ...
        assertEquals("CounterBox must be created with correct texture",
            "gfx/layout/counter", counterBox.getTexture().getName());
    }
    ...
    {
        assertEquals(
            "CounterBox playerOne must be created with correct X position",
            120, counterBox.getPosition().getX(), 0.0001f);
        assertEquals(
            "CounterBox playerOne must be created with correct y position",
            492, counterBox.getPosition().getY(), 0.0001f);

        counterBox = CounterBox.createForPlayerTwo(engine);

        assertEquals(
            "CounterBox playerOne must be created with correct X position",
            521, counterBox.getPosition().getX(), 0.0001f);
        assertEquals(
            "CounterBox playerOne must be created with correct y position",
            492, counterBox.getPosition().getY(), 0.0001f);
    }
    ...
    public void testIsDrawnWithCorrectRectangle()
    {
        ...
        assertEquals("COunterBox drawn with wrong Rectangle", new Rectangle(0,
            0, 171, 58), ((MockEngine)engine).getImageRect());
    }
