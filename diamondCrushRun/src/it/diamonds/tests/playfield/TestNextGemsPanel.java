...
public class TestNextGemsPanel extends AbstractGridTestCase
{
    ...
    public void setUp() throws IOException
    {
        ...
        gemQueue = GemQueue.createForTesting(environment,
    ...
    public void testDrawTwoGemPositionAndTexture()
    {
        ...
        assertEquals("gfx/droppables/" + secondGemType.getName() + "/"
            + secondGemColor.getName(),
            ((MockEngine)environment.getEngine()).getImage(1).getName());

        assertEquals(292f,
            ((MockEngine)environment.getEngine()).getQuadPosition(1).getX());
        assertEquals(32f,
            ((MockEngine)environment.getEngine()).getQuadPosition(1).getY());

        assertEquals(292f,
            ((MockEngine)environment.getEngine()).getQuadPosition(0).getX());
        assertEquals(64f,
            ((MockEngine)environment.getEngine()).getQuadPosition(0).getY());
