...
public class Background implements DrawableInterface
{
    ...
    public Background(Environment environment, String textureName, String type)
    {
        Rectangle area = new Rectangle(0, 0,
            environment.getConfig().getInteger("width") - 1,
            environment.getConfig().getInteger("height"));

        background = new Sprite(0, 0, area,
            environment.getEngine().createImage(textureName + type));
    }
